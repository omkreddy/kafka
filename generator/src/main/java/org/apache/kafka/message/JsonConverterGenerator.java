...
public final class JsonConverterGenerator implements MessageClassGenerator {
    ...
    private void generateVariableLengthTargetFromJson(Target target, Versions curVersions) {
        if (target.field().type().isString()) {
            ...
            if (structRegistry.isStructArrayWithKeys(target.field())) {
                buffer.printf("%s _collection = new %s(%s.size(), 0);%n", type, type, target.sourceVariable());
            } else {
                headerGenerator.addImport(MessageGenerator.MESSAGE_UTIL_CLASS);
                buffer.printf("%s _collection = MessageUtil.newArrayList(%s.size(), 0);%n", type, target.sourceVariable());
            }
